...
## Запуск:
Клонируете репозиторий, запускаете pip install -r requirements.txt (установятся numpy, pandas, spacy и библиотека ru_core_news_lg ~500mb)  

В папку с файлами копируете файл test_data.csv (файл должен быть с таким названием, или нужно поменять в main.py), запускаете ./main.py  

Результат сохраняется а **файл res_data.csv**
